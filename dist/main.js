...
let timeout = 10;
let appUpdater = undefined;
let updaterInterval;
let cachedImageData = undefined;
...
function watchSmbSettingsFile(mainWindow) {
...
function getTimeoutFromRefreshRate(refreshRate) {
...
function setupElectronUpdater() {
    ...
    autoUpdater.on('update-not-available', (info) => mainWindow?.webContents.send('update-not-available', { version: info.version }));
