...
import { currentSmbSettings } from ".";
import * as os from 'os';
export let remoteSettings = undefined;
const defaultRemoteSettings = {
    refreshRate: 60 * 1000, // Default to 60 seconds
};
let smbClient;
const IMAGE_EXTS = /\.(jpe?g|png|gif|bmp|webp)$/i;
export async function loadSmbImage(currentFileName) {
...
async function initSmb() {
    const smbSettings = currentSmbSettings;
...
/// </summary>
function normalizeSmbPath(dir, fileName) {
...
export async function loadRemoteSettings(dir) {
    ...
    try {
        remoteSettings = JSON.parse(settingsFile.toString());
...
async function readImageAsBase64(dir, fileName) {
